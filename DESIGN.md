...
## FINDING
Bench, 14 September 2026 (frequency behavior verified by radio config, same date):

Two environments, two questions. The field trial spans two fundamentally different RF environments, and it is important not to confuse what each can measure.

San Diego (light load). The deployed San Diego population is ~99% LongFast. Measured directly: a co-located dual-cohort station heard 247 distinct LongFast nodes versus 2 ShortTurbo nodes over 16 hours. This installed-base asymmetry has two consequences. First, cohort A (LongFast) participates in a real, populated network, while cohort B (ShortTurbo) is nearly alone. Its logs are dominated by the fleet's own probes because almost no ambient ShortTurbo traffic exists to receive.

Second, the two cohorts are not on the same frequency. Presets are not merely different modulation schemes on a shared channel. In Meshtastic's default configuration (channelNum=0, auto), the operating frequency is derived from a bandwidth-dependent division of the band. LongFast (250 kHz bandwidth) and ShortTurbo (500 kHz bandwidth) divide the US band into different numbers of slots (~104 vs ~52), so the same channel name computes to different center frequencies. The two cohorts therefore occupy different, separately-populated slices of spectrum. They neither decode nor significantly interfere with each other. This is confirmed by measured utilization. Cohort A's frequency runs 8–39% utilized (the LongFast population), while cohort B's frequency reads ~0% (almost no deployed ShortTurbo users). Practical consequences are that a station's own A and B radios do not self-interfere, so simultaneous dual-cohort operation from one station is clean, and cohort B's probes are not contended by LongFast traffic, because that traffic is on a different frequency. ShortTurbo genuinely gets a quiet slot, but not because presets are isolated by protocol, but because the auto-frequency mechanism places them on different frequencies with different populations.

What San Diego can measure, then, is propagation, range, and cross-aperture reception under light load. Like, how far and how reliably probes travel between separated apertures (Palomar, Carmel Valley, Long Beach) on each preset. Plus the installed-base asymmetry itself, which is a real finding. Anyone saying "LongFast outperforms ShortTurbo" comparison is dealing with not only by LongFast having vastly more peers, but also by the two presets sitting on differently-loaded frequencies. What San Diego cannot measure is congestion effects, because its channel is not saturated when logging from a suburb. The 247 LongFast nodes are mostly distant, low-duty-cycle beacons, not a collision-inducing load.

DEF CON (heavy load). The congestion question, where ALOHA/backoff regime where preset choice under a saturated channel actually matters, becomes measurable only where utilization approaches the collision knee, which requires the density of a large event. This is precisely the environment the San Diego field trial cannot replicate and DEF CON provides. There, cohort A and cohort B probes contend with genuinely loaded frequencies, and the delivery difference reflects how each modulation fares under congestion. But note the problems that we have in San Diego do not vanish at DEFCON. they get more problematic. An A-vs-B delivery difference mixes (1) modulation robustness, (2) the two cohorts' different center frequencies, and (3) the different populations/loads on those frequencies. These cannot be fully separated in the wild, because the deployed base fixes LongFast to a busy frequency and ShortTurbo to a sparse one. What DEF CON adds is that both frequencies **may** become loaded enough for congestion to bite. So the comparison is at least "congested vs congested" rather than "busy vs empty." What we are depending on is having something to compare default configurations **against**. Cohort B needs to be something interesting, such as DEFCON-specific firmware. 
