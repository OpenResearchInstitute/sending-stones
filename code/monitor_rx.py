...
def on_receive(self, packet, interface):
    ...
    # hop fields present (the a5fe "hop_start=0 means unknown" doctrine,
    # enforced at capture).
    ...
    # payload length: text if present, else raw decoded bytes if present
    ...
    row = (
        ...
        plen,
        packet.get("rxRssi"),   # None on self/ACK — expected
        packet.get("rxSnr"),    # None on self/ACK — expected
        hops,
    )
    with LOCK:
        self.conn.execute(
